...
def problem2b(rect, n, delta, win):
    ...
    rect.attach_to(win)
    for k in range(n-1):
        top_right = rg.Point((rect.get_upper_right_corner().x + delta*(k+1)), (rect.get_upper_right_corner().y - delta*(k+1)))
        bottom_left = rg.Point((rect.get_lower_left_corner().x - delta*(k+1)), (rect.get_lower_left_corner().y + delta*(k+1)))
        rect_n = rg.Rectangle(top_right,bottom_left)
        rect_n.attach_to(win)
    win.render()
